...
public class Reusable_Actions_Loggers_POM {
    ...
    public static void multipleMouseHover(WebDriver driver, WebElement xpath, Integer number, ExtentTest logger, String elementName) throws InterruptedException {
        //declare wait
        Thread.sleep(3000);
        ...
        logger.log(LogStatus.INFO,"Hovering on element " + elementName);
        try{
            //WebElement sendTab = driver.findElements((xpath)).get(number);
            //WebElement sendTab = ExpectedConditions.visibilityOfAllElements((xpath)).get(number);
            WebElement sendTab = wait.until(ExpectedConditions.visibilityOfAllElements((xpath))).get(number);
    ...
    }//end of working multiple mouse hover method
